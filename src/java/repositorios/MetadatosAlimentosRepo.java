...
public interface MetadatosAlimentosRepo extends JpaRepository<MetadatosAlimentosG, Integer> {
    ...
    @Query("SELECT r FROM MetadatosAlimentosG r WHERE "
            + "r.idAlimento=?1 "
            + "and r.idRegion=?2 "
            + "and r.idEpoca=?3 "
            + "and r.idNivelFert=?4 "
            ...
            + "and r.n=?6 "
            + "and r.npk=?7 "
            + "and r.idRangoEdades=?8 "
            + "and r.corte like ?9 "
            + "and r.tecnolog like ?10 "
            + "and r.tratamiento like ?11 "
            + "and r.presentation like ?12 "
            + "and r.calidad=?13 "
            + "and r.import1=?14 "
            + "and r.fertilizado=?15")
    ...
    @Query("SELECT r FROM MetadatosAlimentosG r WHERE r=?1")
    public List<MetadatosAlimentosG> metadatosAlimentosDeLaRegion(MetadatosAlimentosG alimentosG);
}