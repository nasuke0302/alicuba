                    ...
                    for (int met = 0; met < chequeo_metadatos.size(); met++) {
                        ...
                        if (alimentoEval.isEmpty()) {
                            ...
                            List<MetadatosAlimentosG> metadatosEvaluado = metadatosAlimentosRepo.metadatosAlimentosDeLaRegion(
                                    ...
                                    chequeo_metadatos.get(met).getRiego(),
                                    chequeo_metadatos.get(met).getN(),
                                    chequeo_metadatos.get(met).getNpk(),
                                    chequeo_metadatos.get(met).getIdRangoEdades(),
                                    chequeo_metadatos.get(met).getCorte(),
                                    chequeo_metadatos.get(met).getTecnolog(),
                                    chequeo_metadatos.get(met).getTratamiento(),
                                    chequeo_metadatos.get(met).getPresentation(),
        ...
        if (chequeo_metadatos.size() == 1) {
            ...
            if (tabla_cna_general.size() > 0) {
                for (int reg = 0; reg < tabla_cna_general.size(); reg++) {
                    float valor = 0;
                    TablaCnaFinal tablaCnaFinal_tmp = new TablaCnaFinal(
                            metadatoAgregado.getIdMetadatosAlimentosTabla(),
                            tabla_cna_general.get(reg).getTablaCnaGeneralPK().getIdNutriente());
            ...
            List<Float> valores = new LinkedList<>();
            for (int nut = 0; nut < listaNutrientes.size(); nut++) {
                ...
                for (int met = 0; met < chequeo_metadatos.size(); met++) {
                    ...
                    TablaCnaGeneralPK cnaGeneralPK = new TablaCnaGeneralPK(
                            chequeo_metadatos.get(met).getIdMetadatosAlimentosG(),
                            listaNutrientes.get(nut).getIdNutriente());
